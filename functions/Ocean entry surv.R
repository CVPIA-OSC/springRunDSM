...
# Ocean.ent.surv<- function(migrants,mnth,T.mo, mo.ocean.surv,int){
#   B0<- int#-2.472915
#   drain<- c(0,0,0,0)
...
Ocean.ent.surv2<- function(migrants,mnth,T.mo, mo.ocean.surv,
                           vec1,vec2,vec3,vec4,vec5,vec6,vec7,stoch,vary,pctil){
  # B0<- -2.472915
  B0<-rep(vec1,31) #Antelope Creek and all other tribs with missing data
  B0[10]<-vec2 #deer creek
  B0[12]<-vec3 # mill creek 
  B0[18:19]<-vec4   #middle sac tribs (Feather River)
  B0[20]<-vec5    #middle sac tribs (Yuba)
  B0[3]<-vec6    #Battle and Clear
  B0[7]<-vec6    #Battle and Clear
  B0[6]<-vec7    #Butte
